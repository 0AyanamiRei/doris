...
ObjectStorageListPage S3ExpressObjStorageClient::list_objects(const ObjectStoragePathOptions& opts,
    ...
    auto page = S3ObjStorageClient::list_objects(list_opts, continuation_token);
...
ObjectStorageResponse S3ExpressObjStorageClient::check_versioning(const std::string& /*bucket*/) {
    // Directory buckets do not support S3 Versioning.
    return ObjectStorageResponse::OK();
}

ObjectStorageResponse S3ExpressObjStorageClient::complete_multipart_upload(
    ...
    for (size_t i = 0; i < completed_parts.size(); ++i) {
        const auto& completed_part = completed_parts[i];
        if (completed_part.part_num != static_cast<int>(i + 1)) {
    ...
    return complete_multipart_upload_impl(opts, std::move(request));
}
